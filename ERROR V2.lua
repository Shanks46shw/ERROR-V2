...
game.StarterGui:SetCore("SendNotification", {
    Title = "هسه رح يفتح يمك";
    Text = "سكربت المطور ERROR V2"; -- TEAM ERROR
    Duration = 5;
})
local args = {
    [1] = "RolePlayBio",
    [2] = "سكربت المطور ERROR V2"
}
...
AddParagraph(Main, {"ERROR قنوات"})
AddButton(Main, {
  Name = "انسخ قناة اليوتيوب",
  Callback = function()
    setclipboard('https://youtube.com/@trunks-script?si=nj3mfYq8LJKQPoJn')
  end
})
AddButton(Main, {
  Name = "انسخ قناة التليجرام",
  Callback = function()
    setclipboard('https://t.me/TEEM_YAT')
  end
})

local Main = MakeTab({Name = "🦅 الرؤوس والارجل 🦅"})
local Paragraph = AddParagraph(Main, {"هنا كل الاجسام والرؤوس", "bom dia meus manos"})
AddButton(Main, {
  Name = "🦅 راس روبوت 🦅",
  Callback = function()
...
local Paragraph = AddParagraph(Main, {"قائمة اجسام البنات فقط لان الاولاد رح اضيفهم بالتحديث الجاي "})
...
Notification.new({
	Description = 'حقوق ERROR';
	Title = "TRUNKS | FLY";
	Duration = 7;
	Icon = "rbxassetid://115881843808544",
})
local Script_Title = "TRUNKS | FLY"
...
local section = AddSection(Main, {"⚜️البايو⚜️"})
